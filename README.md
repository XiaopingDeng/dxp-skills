...
### Step 5: 验证与使用
0. 新建一个文件夹，将待审阅论文（支持doc或docx格式,不支持pdf）放入该文件夹下

1. 在文件资源管理器中，点击顶部的地址栏，输入 cmd 然后按 回车键，打开终端（普通权限即可），输入：
...
可以基于评审过程和需求采用/skill-creator (该skill可从 GitHub 仓库一键安装社区安装)进行/dxp-thesis-reviewer的个性化定制迭代，例如:
```bash
# 斜杠命令方式 TAB选择
...
- 也可将dxp-thesis-reviewer文件夹放到Claude Code的Skill目录下（通常位于C:\Users\admin\.claude\skills下）
